...
#define DEBUG_ARR(X,M,A,N) {if (Debug::doDebug(X)){Debug::debugFile()<<M;for(auto idebug_=0;idebug_<N;idebug_++)Debug::debugFile() << A[idebug_] << " ";Debug::debugFile()<< std::endl; Debug::debugFile().flush();}}
#define DEBUG_VEC(X,M,A) {if (Debug::doDebug(X)){Debug::debugFile()<<M;for(size_t idebug_=0;idebug_<A.size();idebug_++)Debug::debugFile() << A[idebug_] << " ";Debug::debugFile()<< std::endl; Debug::debugFile().flush();}}
#define DEBUG_CNTR(X,M,A) {if (Debug::doDebug(X)){Debug::debugFile()<<M;for(auto debug_iter=A.begin();debug_iter!=A.end();++debug_iter)Debug::debugFile() << *(debug_iter) << " ";Debug::debugFile()<< std::endl; Debug::debugFile().flush();}}
#define DEBUG_PTRVEC(X,M,A) {if (Debug::doDebug(X)){Debug::debugFile()<<M;for(size_t idebug_=0;idebug_<A.size();idebug_++)Debug::debugFile() << *A[idebug_] << " ";Debug::debugFile()<< std::endl; Debug::debugFile().flush();}}
